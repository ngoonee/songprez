...
class TestSongObject(unittest.TestCase):
    ...
    def setUp(self):
        join = os.path.join
        self.baseDir = join(os.path.curdir, 'tests', 'dirStructureTest')
        read = spsong.SPSong.read_from_file
        self.s1 = read(join(self.baseDir, 'Songs', 'Children Of The Light'))
        self.s2 = read(join(self.baseDir, 'Songs',
                            'Jesus You Are My Lord (with Chinese)'))
        self.s3 = read(join(self.baseDir, 'Songs', 'Yesus Pokok (Ohhhh)'))
        self.s4 = read(join(self.baseDir, 'Songs', 'You Reign'))
        pass

    def tearDown(self):
        pass

    def test_split_slides(self):
        retval = self.s1.split_slides()
        pass
